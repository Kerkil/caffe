...
void Net<Dtype>::Init(const NetParameter& in_param) {
  ...
  NetParameter filtered;
  FilterNet(in_param, &filtered);
  int solver_count = Caffe::solver_count();
  // Divide batch size by number of solvers for parallel case
  if (phase_ == TRAIN) {
    for (int i = 0; i < filtered.layer_size(); ++i) {
      string m = "Batch size must be divisible by the number of solvers (GPUs)";
      if (filtered.layer(i).has_data_param()) {
        if (filtered.layer(i).data_param().has_batch_size()) {
          uint32_t total = filtered.layer(i).data_param().batch_size();
          uint32_t batch = total / solver_count;
          CHECK(batch * solver_count == total) << m;
          filtered.mutable_layer(i)->mutable_data_param()->set_batch_size(
              batch);

          // Also adjust the prefetch count, as it is shared by all solvers
          uint32_t prefetch = filtered.layer(i).data_param().prefetch();
          filtered.mutable_layer(i)->mutable_data_param()->
              set_prefetch(prefetch * solver_count);
        }
      }
      if (filtered.layer(i).has_hdf5_data_param()) {
        if (filtered.layer(i).hdf5_data_param().has_batch_size()) {
          uint32_t total = filtered.layer(i).hdf5_data_param().batch_size();
          uint32_t batch = total / solver_count;
          CHECK(batch * solver_count == total) << m;
          filtered.mutable_layer(i)->mutable_hdf5_data_param()->
              set_batch_size(batch);
        }
      }
      if (filtered.layer(i).has_image_data_param()) {
        if (filtered.layer(i).image_data_param().has_batch_size()) {
          uint32_t total = filtered.layer(i).image_data_param().batch_size();
          uint32_t batch = total / solver_count;
          CHECK(batch * solver_count == total) << m;
          filtered.mutable_layer(i)->mutable_image_data_param()->
              set_batch_size(batch);
        }
      }
      if (filtered.layer(i).has_memory_data_param()) {
        if (filtered.layer(i).memory_data_param().has_batch_size()) {
          uint32_t total = filtered.layer(i).memory_data_param().batch_size();
          uint32_t batch = total / solver_count;
          CHECK(batch * solver_count == total) << m;
          filtered.mutable_layer(i)->mutable_memory_data_param()->
              set_batch_size(batch);
        }
      }
      if (filtered.layer(i).has_window_data_param()) {
        if (filtered.layer(i).window_data_param().has_batch_size()) {
          uint32_t total = filtered.layer(i).window_data_param().batch_size();
          uint32_t batch = total / solver_count;
          CHECK(batch * solver_count == total) << m;
          filtered.mutable_layer(i)->mutable_window_data_param()->
              set_batch_size(batch);
        }
      }
    }
  ...
  if (Caffe::root_solver()) {
    LOG(INFO) << "Initializing net from parameters: " << std::endl
              << filtered.DebugString();
  }
  // Create a copy of filtered_param with splits added where necessary.
  NetParameter param;
  InsertSplits(filtered, &param);
