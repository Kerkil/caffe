...
Solver<Dtype>::Solver(const SolverParameter& param)
    : param_(param), iter_(), current_step_(), net_(), callback_() {
...
template <typename Dtype>
SGDSolver<Dtype>::SGDSolver(const SolverParameter& param)
    : Solver<Dtype>(param) {
  CHECK(Caffe::root_solver());
  PreSolve();
  last_time_ = microsec_clock::local_time();
  last_iter_ = this->iter_;
}
...
void SGDSolver<Dtype>::Iteration() {
  ...
  if (this->param_.display() && this->iter_ % this->param_.display() == 0) {
    ...
    double lapse = (time - last_time_).total_milliseconds() / 1000.0;
    double per_s = (iter - last_iter_) / (lapse ? lapse : 1);
    last_iter_ = iter;
    last_time_ = time;
    LOG(INFO)<< "Iteration " << iter << " (" << per_s << "/s), lr = " << rate;
  }
  ClipGradients();
